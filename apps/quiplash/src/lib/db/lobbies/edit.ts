...
import { and, eq } from '@games/db/orm';
...
export async function startGame(roomCode: string) {
	...
	// Atomic transition: only the first concurrent caller flips waiting → in_progress
	const [response] = await db
		.update(lobbiesTable)
		.set({ status: 'in_progress' })
		.where(and(eq(lobbiesTable.id, lobby.id), eq(lobbiesTable.status, 'waiting')))
		.returning();
