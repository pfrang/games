<script lang="ts">
  ...
  function switchMode(next: 'join' | 'create') {
   ...
   <div class="container">
    ...
    <div class="card">
      <div class="card-inner">
        ...
        <div class="toggle">
          ...
          >CREATE</button>
          <div class="toggle-pill" style:transform={mode === 'create' ? 'translateX(100%)' : 'translateX(0)'}></div>
        </div>
    ...
    <p class="hint">
      {mode === 'join' ? 'Ask your host for the room code' : 'Start a new game and invite your friends'}
    </p>
  </div>
...
<style>
  ...
  @keyframes pulse-glow {
    0%, 100% { filter: drop-shadow(0 0 18px #a78bfa88); }
    50%       { filter: drop-shadow(0 0 36px #a78bfacc); }
  }
  ...
  .card {
    ...
    box-shadow: 0 8px 40px #0007, 0 0 60px #a78bfa22;
  ...
  .input-wrap input:focus {
    border-color: #a78bfa;
    box-shadow: 0 0 0 3px #a78bfa33, 0 0 20px #a78bfa22;
  }
  ...
  .join-btn:hover { transform: translateY(-2px) scale(1.03); box-shadow: 0 8px 30px #7c3aed77; }
  .join-btn.create:hover { box-shadow: 0 8px 30px #0ea5e977; }
  .join-btn:hover::before { opacity: 1; }
  .join-btn:active { transform: translateY(1px) scale(.98); }
  .join-btn:disabled { opacity: .6; cursor: not-allowed; transform: none; box-shadow: none; }
