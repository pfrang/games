<script lang="ts">
  ...
  let createdDate = $derived(
    lobby?.createdAt ? new Date(lobby.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''
  );
</script>
  ...
  <div class="card">
    <div class="card-inner">
      <div class="status-row">
        <span class="status-dot" class:waiting={lobby?.status === 'waiting'} class:playing={lobby?.status === 'playing'}></span>
